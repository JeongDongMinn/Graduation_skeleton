# 이거는 그냥 오픈포즈가 제대로 동작하는지 확인 용도 입니다. 이걸 1번으로 실행 해서 문제 없을 시 test2.py를 실행해 주세요
import numpy as np
import itertools
import threading
import darknet
...
capture_origin = cv2.VideoCapture(0)
...
while cv2.waitKey(1) < 0:  # 아무 키나 누르면 끝난다.
    # 웹캠으로부터 영상 가져옴
    hasFrame, frame = capture_origin.read()
    ...
    for i in range(nPoints):
        for j in range(len(detected_keypoints[i])):
            cv2.circle(frame, detected_keypoints[i][j][0:2], 3, [
    ...
    for i in range(24):
        for n in range(len(personwiseKeypoints)):
            ...
            cv2.line(frame, (B[0], A[0]), (B[1], A[1]),
                     colors[i], 3, cv2.LINE_AA)

    cv2.imshow("Output-Keypoints", frame)

capture_origin.release()  # 카메라 장치에서 받아온 메모리 해제
